...
// ── Helper: detect touch capability ──
...
// ── Custom cursor behaviour (desktop only) ──
if (!isTouchDevice()) {
  ...
  // On touch devices, hide the cursor element completely (it’s already hidden via CSS)
  if (cursor) {
...
const swiper = new Swiper('.dragSwiper', {
  slidesPerView: 'auto',
  spaceBetween: 16,
  grabCursor: isTouchDevice(),   // only show grab cursor on touch devices
  freeMode: {
    enabled: true,
    momentum: true,
    momentumRatio: 0.8,
