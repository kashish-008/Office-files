document.querySelectorAll(".dual-range-slider").forEach((slider) => {
  ...
  const minLimit = parseInt(minS.min);
  const maxLimit = parseInt(minS.max);
  ...
  function updateSlider() {
    ...
    const isPercent = slider.classList.contains("slider-blue");

    if (isPercent) {
      minIn.value = `${minV}%`;
      maxIn.value = maxV >= 30 ? `${maxV}% +` : `${maxV}%`;
    } else {
      minIn.value = `$${minV.toLocaleString()}`;
      maxIn.value =
        maxV >= 5000000
          ? `$${maxV.toLocaleString()} +`
          : `$${maxV.toLocaleString()}`;
...
// bedroom button toggle active class
$(".click-functionality button").on("click", function () {
...
function updateSliderBackground() {
  ...
  const percentage =
    ((compSlider.value - compSlider.min) / (compSlider.max - compSlider.min)) *
    100;
...
el.addEventListener("mousemove", (e) => {
  if (!down) return;
  e.preventDefault();
  el.scrollLeft = left - (e.pageX - el.offsetLeft - x) * 1.5;
});
